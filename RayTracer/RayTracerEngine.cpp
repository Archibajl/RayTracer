...
RayTracerEngine::RayTracerEngine(const VoxelGrid& grid)
    : voxelGrid(grid) {
    LOG_INFO("RayTracer initialized with voxel grid: {}x{}x{}", grid.nx, grid.ny, grid.nz);
}

// Destructor
RayTracerEngine::~RayTracerEngine() = default;

// Generate a ray for a specific pixel
Ray RayTracerEngine::generateRay(const Camera& camera, float u, float v) {
...
// Render a full frame
std::vector<Vec3> RayTracerEngine::render(const Camera& camera, int width, int height) {
...
bool RayTracerEngine::rayTriangleIntersection(
    const Ray& ray,
    const Triangle& tri,
...
bool RayTracerEngine::rayBoxIntersection(
    const Ray& ray,
    const Vec3& boxMin,
    const Vec3& boxMax,
...
// Get voxel index
size_t RayTracerEngine::getVoxelIndex(size_t ix, size_t iy, size_t iz) const {
    return ix + voxelGrid.nx * (iy + voxelGrid.ny * iz);
}

// DDA-like voxel grid traversal with ray tracing
RayHit RayTracerEngine::traverseVoxelGridDDA(const Ray& ray) {
    RayHit result;
    result.hit = false;
