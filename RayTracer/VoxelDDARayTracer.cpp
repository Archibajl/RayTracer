...
class VoxelDDARayTracer : public IRayTracer {

    /**
 * Ray tracer using voxel grid with DDA (Digital Differential Analyzer) traversal
 *
 * This method:
 * - Pre-voxelizes the scene into a 3D grid
 * - Uses DDA to efficiently step through voxels along the ray
 * - Only tests triangles in occupied voxels
 *
 * Pros: Very fast for dense scenes, predictable performance
 * Cons: Memory intensive, not great for sparse scenes
 */

    std::string getMethodName() const override { return "Voxel Grid DDA"; }

    const VoxelGrid& voxelGrid;

    // Constructor
    public : VoxelDDARayTracer(const VoxelGrid& grid)
        :voxelGrid(grid) {
        LOG_INFO("[{}] Initialized with voxel grid: {}x{}x{}",
            getMethodName(), grid.nx, grid.ny, grid.nz);
    }

    // Generate a ray for a specific pixel
    public : Ray generateRay(const Camera& camera, float u, float v) {
    ...
    // Render a full frame
    public : std::vector<Vec3> render(const Camera& camera, int width, int height) {
        ...
        LOG_INFO("[{}] Ray hits: {} / {} ({:.1f}%)", getMethodName(), hitCount, totalRays, 100.0f * hitCount / totalRays);
        return frameBuffer;
    }

    // Main ray tracing function - inline wrapper
    public : RayHit traceRay(const Ray& ray) {
        return traverseVoxelGridDDA(ray);
    }

    // Ray-triangle intersection using Möller–Trumbore algorithm
    public : bool rayTriangleIntersection(
        const Ray& ray,
        const Triangle& tri,
    ...
    public : bool rayBoxIntersection(
        const Ray& ray,
        const Vec3& boxMin,
        const Vec3& boxMax,
    ...
    // Get voxel index
    public : size_t getVoxelIndex(size_t ix, size_t iy, size_t iz) const {
        return ix + voxelGrid.nx * (iy + voxelGrid.ny * iz);
    }

    // DDA-like voxel grid traversal with ray tracing
    public : RayHit traverseVoxelGridDDA(const Ray& ray) {
        RayHit result;
        result.hit = false;
        ...
        // Start point (if ray origin is inside grid, use origin; otherwise use entry point)
        Vec3 startPoint = ray.origin;
        if (tMin > 0.0f) {
            startPoint = add(ray.origin, multiply(ray.direction, tMin));
        }

        // Convert start point to voxel coordinates
        ...
        // tDelta: distance along ray to cross one voxel boundary in each dimension
        float tDeltaX = (ray.direction.x != 0) ? std::abs(voxelGrid.voxelSize.x / ray.direction.x) : std::numeric_limits<float>::max();
        float tDeltaY = (ray.direction.y != 0) ? std::abs(voxelGrid.voxelSize.y / ray.direction.y) : std::numeric_limits<float>::max();
        float tDeltaZ = (ray.direction.z != 0) ? std::abs(voxelGrid.voxelSize.z / ray.direction.z) : std::numeric_limits<float>::max();

        // tMax: distance along ray to next voxel boundary in each dimension
        ...
        while (ix >= 0 && ix < static_cast<int>(voxelGrid.nx) &&
            ...
            // If voxel is occupied, test triangles
            if (voxel.occupied && voxel.triangle_count > 0) {
                ...
                // If we found a hit in this voxel, return it
                if (result.hit) {
                    return result;
                }
            }
